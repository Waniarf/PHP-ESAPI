...
{
    private static $_characterToEntityMap = [];

    private static $_entityToCharacterMap = [];
    ...
    public function decodeCharacter($input)
    {
        ...
        if (mb_substr($input, 1, 1, 'UTF-32') == $this->normalizeEncoding('#')) {
            ...
            [, $ordinalValue] = unpack('N', mb_substr($input, 1, 1, 'UTF-32'));
    ...
    private function _parseNumber($input)
    {
        ...
        for ($i = 2; $i < $inputLength; $i++) {
            // Get the ordinal value of the character.
            [, $ordinalValue] = unpack('N', mb_substr($input, $i, 1, 'UTF-32'));
    ...
    private function _getNamedEntity($input)
    {
        ...
        [, $ordinalValue] = unpack('N', $inputCaseUnchanged);
        ...
        $ordinalValue = null;
        [, $ordinalValue] = unpack('N', $inputCaseLower);
